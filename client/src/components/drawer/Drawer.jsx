...
function Drawer({ useFor, children }) {
  ...
  return (
    <>
      ...
      <main
        className={` fixed top-0 left-0 overflow-hidden min-h-screen   ${
          openDrawer && "bg-[#b1dacb9c] text-left w-full"
        } `}
      >
        <input
          type="checkbox"
        ...
        />
        <aside
          className={`fixed overflow-scroll z-10 w-full min-h-screen -translate-x-full left-0 duration-500 transition-transform  flex inset-y-0 peer-checked:transform-none peer-checked:opacity-100   rounded-none `}
          role="menu"
          id="menu"
